...
1. Describe the key differences between the Creative Commons Zero v1.0 Universal, Open Source MIT License, and Open Source GPL License. <br>
- Creative Commons Zero v1.0 Universal license is the most open Creative Commons license, which dedicates a work completely to the public with the creator giving up all rights. An Open Source MIT License is a free software license that originated in MIT (why it's called MIT License) that allows free permission to use/copy/modify the software without nearly any restrictions. An Open Source GPL License is known as a general public license, and it gives users the right to use a piece of software for their own benefits by asserting copyright on the software and giving this GPL license with permission to copy/distribute/modify it to them, but also limits the money that can be made from the creators of the software. Creative Commons Zero v1.0 Universal license is the least restrictive, then the Open Source MIT License, and the most restrictive out of the three is Open Source GPL License.
...
- Qualcomm uses the Open Source GPL License, and the most widely known example is the linux kernel, which does allow users to modify the kernel, but they must publish it under the same GPL license terms. It's also important to note that the GTP license doesn't monetarily benefit Qualcomm, so they use a model called royalty payment where the license comes with a regular fee. Red Hat contributes to Open Source projects like the linux kernel and uses subscriptions as well to gain revenue.
...
- Business that rely on Open Source software still need to generate income to be able to stay afload as a company, covering regular operational costs (building, light, heat, electricity, water, etc.), wages of employees, and enough money as a cushion for potential issues. The fundamental nature of Open Source generates no profit for the company, so they need to find other ways to do so, and some include subscriptions like Red Hat, royalty payment like Qualcomm, premium versions with additional (but not entirely necessary) features like MySQL, and many more ways.
...
- Selecting an appropriate license for personal and team GitHub repositories are important because it controls the intellectual property rights for the project and covers the legal protection, allowing or banning its use, modification, and distribution by others. For our CPT project, if we find a similar portion of code in a separate project out on the Internet, it's important to ensure that we are allowed to use and modify that piece of code from their project to incorporate into our project as well. Even though this is for school and a class, it's good to create habits of checking the legal protection of other projects and selecting appropriate licenses for our projects so we can legally and ethically grow as coders.
